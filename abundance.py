...
def main(output_dir, input_file=None, output_file=None):
    # Initialize a counter to count the occurrences of each gene
    gene_counter = Counter()

    # Get the list of promoter directories to process
    if input_file is None:
        # If no input file is given, process all of the directories
        promoter_dirs = os.listdir(output_dir)
    else:
        # If an input file is given, read the names from the file and use them to filter the directories
        with open(input_file) as f:
            promoter_dirs = [line.strip() for line in f]

    # Iterate over the promoter directories in the output directory
    for promoter_dir in os.listdir(output_dir):
        if promoter_dir in promoter_dirs:
            ...
            # Iterate over the TF family directories in the promoter directory
            for tf_family_dir in os.listdir(promoter_path):
                tf_family_path = os.path.join(promoter_path, tf_family_dir)
                # Iterate over the peak count files in the TF family directory
    ...
    with open(output_file, 'w') as outfile:
        # Write the gene names and counts to the file
...
if __name__ == '__main__':
    ...
    # Get the input file name from the command line argument (if it was given)
    input_file = sys.argv[2] if len(sys.argv) > 2 else None
    # Get the output file name from the command line argument (if it was given)
